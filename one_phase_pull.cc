...
#include "diffusion3/apps/ping/ping.hh"
#include "diffusion3/apps/ping/wcv.h"
...
void OnePhasePullFilter::gradientTimeout()
{
  ...
  DiffPrintWithTime(DEBUG_ALWAYS, "Node%d: Gradient Timeout !\n",((DiffusionRouting *)dr_)->getNodeId());
				     ...
				     DataForwardingHistory *forwarding_history)
{
  ...
  DiffPrintWithTime(DEBUG_ALWAYS, "Node %d: Sink processing list length : %d\n", ((DiffusionRouting*)dr_)->getNodeId(), sinks_flow_list.size());
  ...
  while (local_flow_list.size() > 0){
    ...
    for (flow_id_itr = local_flow_list.begin();
	 flow_id_itr != local_flow_list.end(); ){
...
void OnePhasePullFilter::processNewMessage(Message *msg)
{
  ...
  case INTEREST:
    ...
    DiffPrintWithTime(DEBUG_ALWAYS, "Node%d: Received Interest -> %d !\n", ((DiffusionRouting*)dr_)->getNodeId(), msg->next_hop_);
	...
	(nrclass->getOp() == NRAttribute::IS)){
      ...
      DiffPrintWithTime(DEBUG_ALWAYS, "Node%d: This is an Interest !\n", ((DiffusionRouting *)dr_)->getNodeId());
    ...
    while (routing_entry){
      // Got a match
      DiffPrintWithTime(DEBUG_ALWAYS, "Node%d: Send Interest to the Sinks!\n",((DiffusionRouting *)dr_)->getNodeId());
	...
	routing_entry->subscription_list_.push_back(subscription_entry);
        DiffPrintWithTime(DEBUG_ALWAYS, "Node%d: Send Interest to the Sink!\n",((DiffusionRouting *)dr_)->getNodeId());
  ...
  case DATA:
    ...
    DiffPrintWithTime(DEBUG_NO_DETAILS, "Node%d: Received Data !\n",((DiffusionRouting *)dr_)->getNodeId());
